...
def read_phd2_events():
    ...
    while True:
        ...
        try:
            ...
            req = {"method": "get_app_state", "id": 1}
            req_line = json.dumps(req) + "\n"
            logger.debug("Sending RPC to PHD2: %s", req)
            ...
            for raw_line in f:
                ...
                if "result" in msg and "id" in msg:
                    logger.info("PHD2 RPC response id=%s result=%s", msg.get("id"), msg.get("result"))
                    # You can inspect app state here if desired
                    continue
                ...
                if evt == "GuideStep":
                    ...
                    ra_raw = msg.get("RADistanceRaw")
                    dec_raw = msg.get("DECDistanceRaw")
                    snr = msg.get("SNR")
                    ...
                    if ra_raw is not None and dec_raw is not None:
                        total_err = math.sqrt(ra_raw**2 + dec_raw**2)
                    else:
                        total_err = None
                    ...
                    logger.info(
                        "GuideStep at %s RA_raw=%s Dec_raw=%s Total=%s dx=%s dy=%s SNR=%s AvgDist=%s",
                        t, ra_raw, dec_raw, total_err, dx, dy, snr, avg_dist
                    )

                    # Publish MQTT states
                    publish_numeric(f"{BASE_TOPIC}/ra_error_arcsec", ra_raw)
                    publish_numeric(f"{BASE_TOPIC}/dec_error_arcsec", dec_raw)
                    publish_numeric(f"{BASE_TOPIC}/total_error_arcsec", total_err)
